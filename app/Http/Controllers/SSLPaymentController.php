...
class SSLPaymentController extends BaseController
{
    public function process($order_id){
        $order = Order::with('order_details','shipping')->where('id', $order_id)->first()->toArray();
        $products_ids = array();
        ...
        $post_data = [
            'store_id'         => 'merlintechbd0live',
            'store_passwd'     => '654C888188F2478272',
            'total_amount'     => $order['grand_total'],
            'currency'         => 'BDT',
            'tran_id'          => $order_id,
            'success_url'      => url('/') . '/order-process/sslcommerz/success',
            'fail_url'         => url('/') . '/order-process/sslcommerz/fail',
            'cancel_url'       => url('/') . '/order-process/sslcommerz/cancel',
            'emi_option' => 0,

            'cus_name'         => $order['shipping']['name'],
            'cus_email'        => $order['shipping']['mobile_no'],
            ...
            'cus_phone'        => $order['shipping']['mobile_no'] ,
...
//            echo "<meta http-equiv='refresh' content='0;url=".$sslcz->GatewayPageURL."'>";
    ...
    {
        $this->setPageTitle('Payment Failed', 'Failed');
        return view('order-fail');
    }

    public function cancel(Request $request)
    {
        $this->setPageTitle('Payment Canceled', 'Canceled');
        return view('order-fail');
    }
}
